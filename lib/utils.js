...
function createTable(db, options, cb) {
  var table     = options.table
    ...
    toArray(options.fields).forEach(function createField(f, i) {
      var field = []
      ...
      else if (f.index === 'INDEX')   index.push(field[0])
      else if (f.index === 'UNIQUE')  field.push('UNIQUE')
    ...
    if (index.length)   fields.push('INDEX ('+       index  .join(', ') +')')
